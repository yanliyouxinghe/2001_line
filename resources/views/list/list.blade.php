      ...
      <ul>
         ...
         <dt>按品牌筛选：</dt>    
               @foreach($brandInfo as $v)
                  <dd class="searc" field="brand_id" value="{{$v['brand_id']}}" title="{{$v['brand_name']}}">
                     <a @if(isset($query['brand_id']) && $query['brand_id']==$v['brand_id']) class="redhover" style="border :solid 1px #ff0000;color:#fff;background-color:#ff0000" @endif>{{$v['brand_name']}}</a>
                  </dd>
            @endforeach
         </dl>
         </li>
         ...
         @foreach($priceInfo as $v)
               <dd class="searc" field="shop_price" value="{{$v}}"> 
                  <a @if(isset($query['shop_price']) && $query['shop_price']==$v) class="redhover" style="border :solid 1px #ff0000;color:#fff;background-color:#ff0000"  @endif>{{$v}}</a>
               </dd>
            @endforeach
         </dl>
         </li>
...
<script src="/static/js/jquery.js"></script>
   ...
           });
      ...
      $(document).on('click','.delhistory',function(){
         $.post('https://api.winnel.top/delhistorys',function(res){
            if(res.code==0){
               alert(res.msg);
               location.reload()
            }
